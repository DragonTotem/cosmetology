...
import com.zbt.common.network.interceptor.logging.LogInterceptor.Companion.isJson
import com.zbt.common.network.interceptor.logging.LogInterceptor.Companion.isXml
import com.zbt.common.util.CharacterHandler.Companion.jsonFormat
import com.zbt.common.util.CharacterHandler.Companion.xmlFormat
import com.zbt.common.util.LogUtils
...
class DefaultFormatPrinter : FormatPrinter{
    ...
    companion object {
         ...
         *
         *
         * 此问题引起的原因, 据 JessYan 猜测, 可能是因为 AndroidStudio v3.1 以上将极短Time内以相同 tag 输出多次的 log 自动合并为一次输出
         * 导致本来对称的输出日志, 出现不对称的问题
         * AndroidStudio v3.1 此次对输出日志的优化, 不小心使市面上所有具有日志格式化输出功能的日志框架无法正常工作
         * 现在暂时能想到的解决方案有两个: 1. 改变每行的 tag (每行 tag 都加一个可变化的 token) 2. 延迟每行日志打印的间隔Time
         *
         *
